...
namespace QSpace
{
	...
	public:
		...
		Matrix(const Matrix<Type1, rows, cols>& mat)
		{
			for (int i = 0; i < rows * cols; i++)
			{
				data[i] = (Type)mat.data[i];
			}
		}
		...
		inline Matrix& operator << (Type num)
		{
			data[rows * cols - 1] = 0;
			data[0] = num;
		...
		{
			int index = (int)(data[rows * cols - 1] + 0.25f);

			if (index < rows * cols - 1) data[++index] = num;
			if (index < rows * cols - 1) data[rows * cols - 1] = (Type)index;
		...
		inline bool InvFrom(const Matrix<Type, cols, cols>& mat)
		{
			...
			for (pivot_col = 0; pivot_col < cols; ++pivot_col)
			{
				...
				while (pivot_row < rows)
				{
					...
					{
						QPrint("Error In InvFrom:Div Zero\r\n");
						return false;
					}
					p_mat1 += cols;
				}
		...
		template<typename Type1, int rows_b, int cols_b>
		inline void SetBlock(int row, int col, const Matrix<Type1, rows_b, cols_b>& mat)
		{
		...
		inline void Print(const char* format = "% .3e") const
		{
	...
	template<typename Type1, typename Type2, int rows, int cols>
	inline void operator += (Matrix<Type1, rows, cols>& mat1, const Matrix<Type2, rows, cols>& mat2)
	{
		mat1.AddBy(mat2);
	}
	template<typename Type, int rows, int cols>
	...
	template<typename Type1, typename Type2, int rows, int cols>
	inline void operator *= (Matrix<Type1, rows, cols>& mat1, const Matrix<Type2, cols, cols>& mat2)
	{
		mat1.MulBy(mat2);
	}
	template<typename Type, int rows, int cols>
